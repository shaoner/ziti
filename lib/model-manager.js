...
ModelManager.prototype.addModel = function (model) {
    ...
    // Add methods
    var fnName;
    for (fnName in this._statics) {
        if (!(fnName in model._statics)) {
            model.setStatic(fnName, this._statics[fnName]);
        }
    }
    for (fnName in this._methods) {
        if (!(fnName in model._methods)) {
            model.setMethod(fnName, this._methods[fnName]);
        }
    }
};
