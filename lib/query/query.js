...
Query.prototype._transformExtraWhere = function (result, value, key) {
    result[this._name + '.' + key] = value;
};
...
Query.prototype.at = function (where) {
    if (where && _.isPlainObject(where)) {
        var refs = _.keys(this._model._references);
        this._extraWhere = _.transform(_.pick(where, refs),
                                       this._transformExtraWhere, { }, this);
        this._where = _.omit(where, refs);
    } else {
...
Query.prototype._addJoin = function (leftName, rightName, reference) {
    ...
    if (!(rightName in structure)) {
        if (reference._throughTarget) {
            ...
            if (intRightName in this._extraWhere) {
                structure[intRightName].where = this._extraWhere[intRightName];
            }
            ...
            if (rightName in this._extraWhere) {
                structure[rightName].where = this._extraWhere[rightName];
            }
 ...
 */
Query.prototype._setFieldFromAttribute = function (model, name, attr, parent) {
    var rAttr = attr.split('.'), attrName, ref, i, len;
    attr = rAttr.shift();
    if (rAttr.length < 1) {
        if (attr in model._core) {
            ...
            if (!(name in this._fields)) {
                ...
                this._fields[name].reference = parent;
            }
            ...
            for (i = 0, len = subAttrs.length; i < len; ++i) {
                this._setFieldFromAttribute(ref._target, attrName, subAttrs[i], ref);
            }
        }
    } else {
        ...
        if (attr in model._references) {
            ...
            ref = model._references[attr];
            this._addJoin(name, attrName, ref);
            var key = ref._target._pk.length > 0 ? ref._target._pk : ref._target._attributes;
            for (i = 0, len = key.length; i < len; ++i) {
                this._setFieldFromAttribute(ref._target, attrName, key[i], ref);
            }
            this._setFieldFromAttribute(ref._target, attrName, rAttr.join('.'), ref);
        }
    }
};
...
Query.prototype._formatMultiRaw = function (rows, name) {
    ...
    for (var i = 0, len = groups.length; i < len; ++i) {
        ...
        for (var attr in this._fields) {
            ...
            if (attr === name || !(attr in group[0]) || attrName.indexOf('.') > -1) {
                continue;
            }
            var reference = this._fields[attr].reference;
            if (reference._unique === true) {
                tmp = group[0][attr];
                _.set(o, attrName, isEmpty(tmp) ? null : tmp);
