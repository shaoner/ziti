...
Query.prototype._formatRows = function (rows) {
    ...
    var o = { }, keys = this._keys, model = this._model, tmp;
    _.assign(o, rows[0][this._name]);
    for (var name in keys) {
        if (name in model._references) {
            if (model._references[name]._unique === true) {
                tmp = rows[0][name];
                o[name] = _.every(tmp, _.isNull) ? null : tmp;
            } else {
                tmp = _.reduce(rows, function (result, item) {
                    item = item[name];
                    if (item && !_.every(item, _.isNull)) {
                        result.push(item);
                    }
                    return result;
                }, [ ]);
                o[name] = _.uniq(tmp, function (item) {
