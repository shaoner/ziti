...
var ModelInstance = function (model, row, options) {
    ...
    this._newValues = { };
    this._references = { };
    this._setMethods(model._methods);
...
ModelInstance.prototype._fillValues = function (data) {
    ...
    for (attr in model._core) {
        if (attr in this._realValues) {
            continue;
        }
        value = model._core[attr]._default;
...
ModelInstance.prototype.get = function (name) {
    if (name) {
        ...
        if ((name in core) && core[name]._getter) {
            return core[name]._getter.bind(this)();
 ...
 * @param {string|Number|ModelInstance|Object} [value] - The value of the field
 */
