...
QueryBuilder.insert = function (table, data) {
    var sql, values = [ table ];
    if (_.isPlainObject(data)) {
        sql = 'INSERT INTO ?? SET ';
        var expr = QueryBuilder.parseSet(data);
        return { sql: sql + expr.sql, values: values.concat(expr.values) };
    }
...
QueryBuilder.update = function (table, name, data, options) {
    var expr = QueryBuilder.parseSet(data);
    var sql = 'UPDATE ?? ?? SET ' + expr.sql;
    var values = [ table, name ].concat(expr.values);
...
QueryBuilder.upsert = function (table, data) {
    return {
        sql: 'INSERT INTO ?? SET ? ON DUPLICATE KEY UPDATE ?',
        values: [ table, data, data ]
    };
};
...
QueryBuilder.sum = function (table, column, where) {
    var sql = 'SELECT SUM(??) AS $sum FROM ??', values = [ column, table ];
    if (where) {
        var expr = QueryBuilder.parseExpr(table, where);
        sql += ' WHERE ' + expr.sql;
        values = values.concat(expr.values);
    }
    return { sql: sql, values: values };
...
QueryBuilder.min = function (table, column, where) {
    var sql = 'SELECT MIN(??) AS $min FROM ??', values = [ column, table ];
    if (where) {
        var expr = QueryBuilder.parseExpr(table, where);
        sql += ' WHERE ' + expr.sql;
        values = values.concat(expr.values);
...
QueryBuilder.max = function (table, column, where) {
    var sql = 'SELECT MAX(??) AS $max FROM ??', values = [ column, table ];
    if (where) {
        var expr = QueryBuilder.parseExpr(table, where);
        sql += ' WHERE ' + expr.sql;
        values = values.concat(expr.values);
...
QueryBuilder.count = function (table, where) {
    var sql = 'SELECT COUNT(*) AS $count FROM ??', values = [ table ];
    if (where) {
        var expr = QueryBuilder.parseExpr(table, where);
        sql += ' WHERE ' + expr.sql;
        values = values.concat(expr.values);
...
QueryBuilder.parseSet = function (data) {
    var sql = [ ], values = [ ];
